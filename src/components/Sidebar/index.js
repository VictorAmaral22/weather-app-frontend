...
export default function Sidebar({selectedCity, setSelectedCity, cityName, setCityName, setCityHistory}) {

    const [cityOptions,
        setCityOptions] = useState([])
        ...
        <form onSubmit={(e) => FindCity(e)} className="searchForm">
            ...
            {selectedCity && (
                <div className='cityInfo'>
                    <div>
                        ...
                        <h4 className='cityDetails'>{selectedCity.state}
                            - {selectedCity.country}</h4>
                    </div>
                    <div
            ...
            {!selectedCity && (
                <div className='placesDiv'>
                    {cityOptions && cityOptions.length > 0 && cityOptions.map((item, key) => {
                        return (
                            <div key={key} className='placeRow' onClick={() => setSelectedCity(item)}>
                                <p>{item.name}, {item.state}
                                    - {item.country}</p>
                            </div>
                        )
                    })}
